...
Multi-tenant distributed task queue. Jobs and idempotency keys live in Postgres (single source of truth), Redis Streams handles the hot transport, and a React dashboard subscribes to live status over WebSocket. Submit p95 around 70 ms, sustained 187 RPS on a 4-vCPU dev VM. Per-tenant API keys, token-bucket rate limits, Redis-backed concurrency quotas, Prometheus + Grafana + OTel traces, structured JSON logs with MDC.
...
- [docs/TESTING.md](docs/TESTING.md): test stack and reproduction commands.
